...
/** 合并的外部包 **/
const vendorJs = ['react', 'react-router', 'axios']

/** 合并入commons的包被引用的最少次数(超过此数字则提取出合入commons文件) **/
const commonsMinChunks = 5
 ...
 **/
const publicPath = isDist ? '/fp_dist/' : '/'
...
const alias = {
  ...
  core: `${srcPath}/core/`,
  util: `${srcPath}/core/util/`,
  net: `${srcPath}/core/net/`,
  common: `${srcPath}/modules/shared/common/`,
  shared: `${srcPath}/modules/shared`,
...
/** 默认端口号 **/
const defaultPort = _argJson.port || 8180
...
/** webpack打包输出 **/
const output = {
  path: path.join(basePath, './fp_dist/'),
  filename: fileName,
...
const indexHtml = new HtmlWebpackPlugin({
  ...
  chunks: ['commons', 'vendor', entryJsName]
})
  ...
  postLoaders: [
    {
      ...
      include: [
        ...
        path.join(nodeModulesPath, 'axios'),
        path.join(nodeModulesPath, 'scriptjs'),
